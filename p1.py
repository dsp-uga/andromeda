...
def predict(rdd_test_data, rdd_train):
    ...
    # (doc, pred)
    rdd_pred = rdd_doc_lablogp.map(lambda x: (x[0],(x[1][1],x[1][0]))).reduceByKey(max).sortByKey().map(lambda x: x[1][1])

    return rdd_pred.collect()
#    return rdd_test_data
...
if __name__ == "__main__":
    ...
    outpath_test = os.path.join('COMP/Practicum/p1/', 'pred_test_' + args['size'] + '.json')
